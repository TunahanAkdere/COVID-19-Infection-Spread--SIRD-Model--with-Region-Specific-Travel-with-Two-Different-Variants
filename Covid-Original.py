...
R0_values = np.random.uniform(2.4, 3.4, size=len(regions))  # Gerçekçi R0 (bulaşma)
gamma = np.random.uniform(0.14, 0.2, size=len(regions))  # İyileşme oranı
...
iterations = 10  # Monte Carlo simülasyon sayısı

# Seyahat matrisi oluştur
...
fig = px.line(
    ...
    y='Infected',
    color='Region',
    title='COVID-19 Infection Spread (SIRD Model) with Region-Specific Travel',
