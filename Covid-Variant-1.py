...
initial_S = populations.copy()  # Tüm nüfus başlangıçta duyarlı (S)
...
R0_values = np.random.uniform(5, 6, size=len(regions))
gamma = np.random.uniform(0.05, 0.1, size=len(regions))
beta = R0_values * gamma  
...
fig = px.line(
    ...
    y='Infected',
    color='Region',
    title='COVID-19 Infection Spread (SIRD Model) with Region-Specific Travel - Variant - 1',
